...
def mlx_pad_reflect_1d_fixed(x, padding_left, padding_right):
    """
    修复的MLX reflect padding，完全匹配PyTorch SConv1d的行为
...
def fixed_mlx_wavenet_forward(mlx_wavenet, x, x_mask, g=None):
    ...
    print(f"初始化output形状: {output.shape}")
    ...
    for i in range(mlx_wavenet.n_layers):
        ...
        print(f"Layer {i}: x_in形状: {x_in.shape}")
        ...
        if i < mlx_wavenet.n_layers - 1:
            ...
            print(f"Layer {i}: res_acts形状: {res_acts.shape}, skip_acts形状: {skip_acts.shape}")
            ...
            print(f"Layer {i}: 最后一层output形状: {output.shape}")
...
def test_final_fix():
    ...
    n_channels_tensor = torch.IntTensor([512])
    ...
    for i in range(8):
        ...
        if g_torch is not None:
            cond_offset = i * 2 * 512
            pytorch_g_l = pytorch_g_cond[:, cond_offset:cond_offset + 2 * 512, :]
        else:
        ...
        if i < 8 - 1:
            pytorch_res_acts = pytorch_res_skip_acts[:, :512, :]
            pytorch_x = (pytorch_x + pytorch_res_acts) * x_mask_torch
            pytorch_output_manual = pytorch_output_manual + pytorch_res_skip_acts[:, 512:, :]
        else:
            pytorch_output_manual = pytorch_output_manual + pytorch_res_skip_acts
        
        print(f"Layer {i}: x range=[{torch.min(pytorch_x):.6f}, {torch.max(pytorch_x):.6f}], output range=[{torch.min(pytorch_output_manual):.6f}, {torch.max(pytorch_output_manual):.6f}]")
    
    pytorch_final = pytorch_output_manual * x_mask_torch
    print(f"PyTorch手动计算最终输出: range=[{torch.min(pytorch_final):.6f}, {torch.max(pytorch_final):.6f}]")
