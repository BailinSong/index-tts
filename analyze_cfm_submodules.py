...
def analyze_cfm_submodules():
    """分析 CFM 子模块差异"""
    print("🔍 CFM Submodule Analysis")
    print("="*60)
    ...
    pytorch_data = collect_pytorch_submodule_data(cached_inputs)
    
    # 收集 MLX 子模块数据
    print("\n🔍 Collecting MLX Submodule Data:")
    mlx_data = collect_mlx_submodule_data(cached_inputs)
    
    # 对比分析
    print("\n📊 Submodule Comparison Analysis:")
    comparison_results = compare_submodules(pytorch_data, mlx_data)
    
    # 生成分析报告
    print("\n📄 Generating Analysis Report:")
    generate_submodule_analysis_report(cached_inputs, comparison_results)
...
def collect_pytorch_submodule_data(cached_inputs):
    """收集 PyTorch 子模块数据"""
    try:
        # 导入 PyTorch CFM
        ...
        with torch.no_grad():
            output = pytorch_cfm.solve_euler(
                ...
                f0=None,
                t_span=t_span,
                debug_layers=False  # 暂时关闭调试，先确保基本功能
            )
...
def collect_mlx_submodule_data(cached_inputs):
    """收集 MLX 子模块数据"""
    try:
        # 导入 MLX CFM
...
def compare_submodules(pytorch_data, mlx_data):
    """对比子模块数据"""
...
def generate_submodule_analysis_report(cached_inputs, comparison_results):
    """生成子模块分析报告"""
    report_lines = []
    report_lines.append("# CFM 子模块对比分析报告")
    ...
    if comparison_results:
        ...
        if comparison_results['severity'] == "严重":
            ...
            report_lines.append("")
            report_lines.append(f"MLX 和 PyTorch 版本存在严重差异 (最大差异: {comparison_results['max_diff']:.2e})，")
            ...
            report_lines.append("")
            report_lines.append(f"MLX 和 PyTorch 版本存在中等差异 (最大差异: {comparison_results['max_diff']:.2e})，")
            ...
            report_lines.append("")
            report_lines.append(f"MLX 和 PyTorch 版本差异微小 (最大差异: {comparison_results['max_diff']:.2e})，")
        ...
        if comparison_results['severity'] in ["严重", "中等"]:
    ...
    if comparison_results:
        if comparison_results['mlx_clipping']:
            ...
            report_lines.append("")
            report_lines.append("**长期目标**: 确保 MLX 和 PyTorch 版本的数值一致性，")
            report_lines.append("使输出差异小于 e-5。")
        else:
            report_lines.append("**主要发现**: MLX 和 PyTorch 版本存在数值差异，")
    ...
    # 保存报告
    report_path = Path("cfm_debug_outputs/cfm_submodule_comparison_report.md")
...
def main():
    """主函数"""
    print("🔍 CFM Submodule Comparison Analysis")
    print("="*60)
    
    try:
        # 分析 CFM 子模块
        analyze_cfm_submodules()
        
        print("\n🎉 Analysis completed successfully!")
        print("\n📋 Generated files:")
        print("1. cfm_debug_outputs/cfm_submodule_comparison_report.md - CFM 子模块对比分析报告")
        
        print("\n📊 Key Features:")
        print("   🔍 使用 CFM 前级缓存作为输入")
        print("   📊 对比 MLX 和 PyTorch 版本 CFM 输出")
        print("   🎯 识别存在差异的子模块")
        print("   💡 提供针对性修复建议")
