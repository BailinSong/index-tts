        ...
        for seg_idx, sent in enumerate(segments):
            ...
            with torch.no_grad():
                with torch.amp.autocast(text_tokens.device.type, enabled=self.dtype is not None, dtype=self.dtype):
                    ...
                    # 调试：记录GPT生成前的输入
                    print(f">> [PyTorch GPT Debug] GPT生成前:")
                    print(f"   spk_cond_emb: {spk_cond_emb.shape}, min={spk_cond_emb.min():.6f}, max={spk_cond_emb.max():.6f}")
                    print(f"   text_tokens: {text_tokens.shape}, min={text_tokens.min():.6f}, max={text_tokens.max():.6f}")
                    print(f"   emo_cond_emb: {emo_cond_emb.shape if emo_cond_emb is not None else None}")
                    print(f"   emovec: {emovec.shape if emovec is not None else None}")
                    ...
                    # 调试：记录GPT生成后的输出
                    print(f">> [PyTorch GPT Debug] GPT生成后:")
                    ...
                    with open('gpt_outputs_torch.pkl', 'wb') as f:
                        pickle.dump(gpt_outputs, f)
                    print(f">> [PyTorch GPT Debug] GPT输出已保存到 gpt_outputs_torch.pkl")
                ...
                with torch.amp.autocast(text_tokens.device.type, enabled=dtype is not None, dtype=dtype):
                    ...
                    # 调试：记录S2MEL处理前的输入
                    print(f">> [PyTorch S2MEL Debug] S2MEL处理前:")
                    print(f"   latent: {latent.shape}, min={latent.min():.6f}, max={latent.max():.6f}")
                    print(f"   codes: {codes.shape}, min={codes.min():.6f}, max={codes.max():.6f}")
                    print(f"   prompt_condition: {prompt_condition.shape}, min={prompt_condition.min():.6f}, max={prompt_condition.max():.6f}")
                    ...
                    # 调试：记录CFM输入
                    print(f">> [PyTorch CFM Debug] CFM输入:")
                    ...
                    with open('cfm_inputs_torch.pkl', 'wb') as f:
                        pickle.dump(cfm_inputs, f)
                    print(f">> [PyTorch CFM Debug] CFM输入已保存到 cfm_inputs_torch.pkl")
                    ...
                    print(f">> [PyTorch S2MEL Debug] S2MEL输入已缓存到 self._s2mel_inputs")
                    ...
                    # 调试：记录CFM输出
                    print(f">> [PyTorch CFM Debug] CFM输出:")
                    ...
                    with open('cfm_outputs_torch.pkl', 'wb') as f:
                        pickle.dump(cfm_outputs, f)
                    print(f">> [PyTorch CFM Debug] CFM输出已保存到 cfm_outputs_torch.pkl")
