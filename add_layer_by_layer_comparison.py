...
def add_layer_comparison_to_cfm():
    """在CFM模块中添加逐层对比功能"""
...
'''
    ...
    if 'def compare_with_pytorch' not in content:
        # 找到MLXCFM类的结束位置
        class_end = content.find('class MLXCFM')
        if class_end != -1:
            # 找到类的最后一个方法
            methods = content[class_end:].split('def ')
            if len(methods) > 1:
...
'''
    ...
    with open('test_layer_comparison.py', 'w', encoding='utf-8') as f:
        f.write(test_script)
    
    print("✓ 已创建测试脚本: test_layer_comparison.py")
...
def main():
    ...
    # 添加对比功能到CFM
    print("\\n📝 添加逐层对比功能到MLX CFM...")
    if add_layer_comparison_to_cfm():
    ...
    print("\\n📋 使用方法:")
    print("1. 运行推理生成缓存数据")
    print("2. 运行测试脚本: python test_layer_comparison.py")
    print("3. 查看逐层对比结果")
