...
def test_rewritten_vs_pytorch():
    """测试重写的MLX CFM vs PyTorch CFM"""
    
    print('=== 测试重写的MLX CFM vs PyTorch CFM ===')
    ...
    try:
        ...
        result_mlx = tts_mlx.infer(
            ...
            text=test_text,
            output_path='rewritten_vs_pytorch_test.wav',
            seed=42
        )
        ...
        if os.path.exists('cfm_inputs_mlx.pkl'):
            ...
            rewritten_vs_pytorch_diff = torch.abs(vc_target_mlx_rewritten_torch - vc_target_pytorch)
            print(f'\\n=== 重写版本 vs PyTorch版本对比 ===')
            print(f'输出差异: 最大 {torch.max(rewritten_vs_pytorch_diff):.6f}, 平均 {torch.mean(rewritten_vs_pytorch_diff):.6f}')
            
            # 检查是否超过阈值
            threshold = 1e-5
            if torch.max(rewritten_vs_pytorch_diff) > threshold:
                print(f'\\n❌ 重写版本与PyTorch版本差异大于阈值 {threshold:.0e}')
                print(f'最大差异位置: {torch.argmax(rewritten_vs_pytorch_diff).item()}')
                print(f'最大差异值: {torch.max(rewritten_vs_pytorch_diff):.6f}')
                
                # 显示差异分布
                large_diff_count = torch.sum(rewritten_vs_pytorch_diff > threshold).item()
                total_elements = rewritten_vs_pytorch_diff.numel()
                print(f'超过阈值的元素数: {large_diff_count} / {total_elements} ({large_diff_count/total_elements*100:.2f}%)')
                
                # 显示前几个大差异的位置
                large_diff_indices = torch.where(rewritten_vs_pytorch_diff > threshold)
                if len(large_diff_indices[0]) > 0:
                    ...
                    for i in range(min(10, len(large_diff_indices[0]))):
                        ...
                        diff_val = rewritten_vs_pytorch_diff[idx].item()
                        print(f'  位置 {idx}: 重写={rewritten_val:.6f}, PyTorch={pytorch_val:.6f}, 差异={diff_val:.6f}')
            else:
                print(f'\\n✅ 重写版本与PyTorch版本差异在可接受范围内 (≤ {threshold:.0e})')
                
                # 计算相关系数
                correlation = torch.corrcoef(torch.stack([vc_target_mlx_rewritten_torch.flatten(), vc_target_pytorch.flatten()]))[0, 1].item()
                print(f'相关系数: {correlation:.6f}')
                
                if correlation > 0.99:
                    print('✅ 重写版本与PyTorch版本高度相关')
                elif correlation > 0.95:
                    print('⚠️  重写版本与PyTorch版本基本相关，存在轻微差异')
                else:
                    print('❌ 重写版本与PyTorch版本相关性较低，存在明显差异')
            
            # 保存详细对比结果
            comparison_results = {
                'rewritten_output': vc_target_mlx_rewritten_torch.detach().cpu(),
                'pytorch_output': vc_target_pytorch.detach().cpu(),
                'difference': rewritten_vs_pytorch_diff.detach().cpu(),
                'max_diff': torch.max(rewritten_vs_pytorch_diff).item(),
                'mean_diff': torch.mean(rewritten_vs_pytorch_diff).item(),
                'rms_diff': torch.sqrt(torch.mean(rewritten_vs_pytorch_diff**2)).item()
            }
            
            with open('rewritten_vs_pytorch_comparison.pkl', 'wb') as f:
                pickle.dump(comparison_results, f)
            print('\\n详细对比结果已保存到 rewritten_vs_pytorch_comparison.pkl')
