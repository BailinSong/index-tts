#!/usr/bin/env python3
"""
对比 PyTorch CFM 和 MLX CFM 的实现差异
"""
...
def compare_cfm_implementations():
    """对比 PyTorch CFM 和 MLX CFM 的实现"""
    
    print('=== 对比 PyTorch CFM 和 MLX CFM 实现 ===')
    ...
    try:
        ...
        result = tts_mlx.infer(
            ...
            text=test_text,
            output_path='mlx_cfm_comparison.wav',
            seed=42
        )
        ...
        # 检查输出文件
        if os.path.exists('mlx_cfm_comparison.wav'):
            audio, sr = librosa.load('mlx_cfm_comparison.wav', sr=None)
            print(f'\\n=== 输出音频信息 ===')
            print(f'文件: mlx_cfm_comparison.wav')
            print(f'采样率: {sr}Hz')
