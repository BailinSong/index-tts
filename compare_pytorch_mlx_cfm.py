...
def compare_pytorch_mlx_cfm():
    """对比 PyTorch 和 MLX CFM 实现"""
    print("🔍 PyTorch vs MLX CFM Comparison")
    print("="*60)
    ...
    pytorch_data = collect_pytorch_cfm_data(cached_inputs)
    
    # 收集 MLX CFM 子流程数据
    print("\n🔍 Collecting MLX CFM Submodule Data:")
    mlx_data = collect_mlx_cfm_data(cached_inputs)
...
def collect_pytorch_cfm_data(cached_inputs):
    """收集 PyTorch CFM 子流程数据"""
    try:
        # 导入 PyTorch CFM
...
def collect_mlx_cfm_data(cached_inputs):
    """收集 MLX CFM 子流程数据"""
    try:
        # 导入 MLX CFM
...
def generate_comparison_report(cached_inputs, pytorch_data, mlx_data, comparison_results):
    """生成对比报告"""
    report_lines = []
    report_lines.append("# PyTorch vs MLX CFM 子流程对比分析报告")
    ...
    # 保存报告
    report_path = Path("cfm_debug_outputs/pytorch_mlx_cfm_comparison_report.md")
...
def main():
    ...
    try:
        ...
        compare_pytorch_mlx_cfm()
        
        print("\n🎉 Comparison completed successfully!")
        print("\n📋 Generated files:")
        print("1. cfm_debug_outputs/pytorch_mlx_cfm_comparison_report.md - PyTorch vs MLX CFM 对比分析报告")
