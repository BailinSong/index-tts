#!/usr/bin/env python3
"""
使用前级输入缓存作为CFM输入来分析差异来源 - 修复版
采用cfm_inputs_torch.pkl为准，使用正确的CFM输入格式
"""
...
def analyze_cfm_with_cached_inputs_fixed():
    """使用前级输入缓存作为CFM输入来分析差异来源 - 修复版"""
    print("=== 使用前级输入缓存作为CFM输入分析差异来源 (修复版) ===\n")
    ...
    if os.path.exists(preferred_cache):
        print(f"   找到PyTorch缓存文件: {preferred_cache}")
        try:
            ...
            print(f"   读取PyTorch缓存文件失败: {e}")
            cached_inputs = None
    
    if cached_inputs is None:
        print("   ❌ 未找到可用的PyTorch缓存输入数据")
        return False
    ...
    # 构建CFM输入 - 使用正确的格式
    print("\n3. 构建CFM输入...")
    # CFM期望的输入格式（根据DiT源码注释）：
    # x: (batch_size, 80, seq_len) - 随机噪声
    # prompt_x: (batch_size, 80, seq_len) - 参考梅尔频谱
    # cond: (batch_size, seq_len, 512) - 语义信息
    
    # 使用ref_mel作为prompt_x
    # ref_mel是(1, 80, 243)，需要调整为(1, 80, 464)
    if ref_mel.shape[2] != 464:
        # 将ref_mel从(1, 80, 243)调整为(1, 80, 464)
        seq_len = ref_mel.shape[2]
        target_len = 464
        if seq_len < target_len:
            # 重复最后几帧来填充
            repeat_times = target_len // seq_len
            remainder = target_len % seq_len
            ref_mel_expanded = ref_mel.repeat(1, 1, repeat_times)
            if remainder > 0:
                # 确保设备一致
                ref_mel_expanded = torch.cat([ref_mel_expanded, ref_mel[:, :, :remainder]], dim=2)
        else:
            # 截断到目标长度
            ref_mel_expanded = ref_mel[:, :, :target_len]
    else:
        ref_mel_expanded = ref_mel
    
    # 使用ref_mel作为prompt_x
    prompt_x = ref_mel_expanded  # (1, 80, 464)
    
    # 生成随机噪声作为x
    x = torch.randn_like(prompt_x)  # (1, 80, 464)
    
    # 使用cat_condition作为cond
    cond = cat_condition  # (1, 464, 512)
    ...
    print(f"   prompt_x: {prompt_x.shape}, 范围: [{prompt_x.min():.6f}, {prompt_x.max():.6f}]")
    print(f"   cond: {cond.shape}, 范围: [{cond.min():.6f}, {cond.max():.6f}]")
    ...
    with open('cfm_cached_inputs_analysis_fixed.pkl', 'wb') as f:
        pickle.dump(analysis_result, f)
    
    print(f"   分析结果已保存到: cfm_cached_inputs_analysis_fixed.pkl")
...
if __name__ == "__main__":
    try:
        success = analyze_cfm_with_cached_inputs_fixed()
        if success:
            print("\n✅ CFM缓存输入分析完成")
        else:
            print("\n❌ CFM缓存输入分析失败")
