...
def compare_first_layer_differences():
    """对比第一层的输入和输出差异"""
    ...
    try:
        ...
        result = tts_mlx.infer(
            ...
            text=test_text,
            output_path='first_layer_test.wav',
            seed=42
        )
        ...
        # 检查输出文件
        if os.path.exists('first_layer_test.wav'):
            import librosa
            audio, sr = librosa.load('first_layer_test.wav', sr=None)
            print(f'\\n=== 输出音频信息 ===')
            print(f'文件: first_layer_test.wav')
            print(f'采样率: {sr}Hz')
...
if __name__ == "__main__":
    compare_first_layer_differences()
