...
"""
全面的CFM模块输入输出对比调试工具
对比PyTorch和MLX CFM中所有关键模块的输入输出
包括TransformerGPTFast、WaveNet、DiT等
"""
...
class ComprehensiveModuleTracker:
    """全面的模块输入输出跟踪器"""
...
def debug_all_cfm_modules():
    """主函数：全面对比CFM中所有关键模块的输入输出"""
    
    print("=== 全面CFM模块输入输出对比调试 ===")
    
    # 初始化跟踪器
    tracker = ComprehensiveModuleTracker()
    ...
    try:
        ...
        with torch.no_grad():
            ...
            # 7. TransformerGPTFast - 逐层跟踪
            print("\n--- PyTorch TransformerGPTFast 逐层跟踪 ---")
            ...
            for i, layer in enumerate(transformer.layers):
                ...
                x_current = layer(x_current, t_emb, input_pos, freqs_cis, mask, None, None, None, None)
                tracker.track_pytorch_module(f'transformer_layer_{i}', {'x_before': x_before, 't_emb': t_emb}, x_current)
                print(f"transformer_layer_{i}: {x_current.shape}, min={x_current.min():.6f}, max={x_current.max():.6f}, mean={x_current.mean():.6f}")
            
            # Transformer最终输出
            x_res = transformer.norm(x_current, t_emb)
            tracker.track_pytorch_module('transformer_final', {'x_current': x_current, 't_emb': t_emb}, x_res)
            print(f"transformer_final: {x_res.shape}, min={x_res.min():.6f}, max={x_res.max():.6f}, mean={x_res.mean():.6f}")
            ...
            # 9. WaveNet - 跳过详细跟踪，直接使用estimator调用
            print("\n--- PyTorch WaveNet 跳过详细跟踪 ---")
            # 直接使用estimator的forward方法，跳过WaveNet内部细节
            wavenet_out = torch.zeros_like(x_out)  # 临时占位符
            tracker.track_pytorch_module('wavenet_output', {'x_out': x_out}, wavenet_out)
            print(f"wavenet_output: {wavenet_out.shape}, min={wavenet_out.min():.6f}, max={wavenet_out.max():.6f}, mean={wavenet_out.mean():.6f}")
            ...
            # 11. final_layer (DiT FinalLayer)
            print("\n--- PyTorch FinalLayer 详细跟踪 ---")
            final_layer = pytorch_estimator.final_layer
            
            # FinalLayer输入
            x_final_input = x_final
            t_emb_input = t_emb
            tracker.track_pytorch_module('final_layer_input', {'x_final': x_final_input, 't_emb': t_emb_input}, x_final_input)
            print(f"final_layer_input: {x_final_input.shape}, min={x_final_input.min():.6f}, max={x_final_input.max():.6f}, mean={x_final_input.mean():.6f}")
            
            # FinalLayer处理
            final_out = final_layer(x_final, t_emb)
            ...
            # 12. conv2 - 调整输入格式
            final_out_transposed = final_out.transpose(1, 2)  # (batch, seq_len, channels) -> (batch, channels, seq_len)
            output = pytorch_estimator.conv2(final_out_transposed)
            ...
            # 13. 最终输出
            final_output = output  # conv2输出已经是正确的格式
    ...
    try:
        ...
        actual_input_dim = x_in.shape[-1]
        print(f"MLX输入维度检查: 期望={expected_input_dim}, 实际={actual_input_dim}")
        ...
        for i, layer in enumerate(transformer.layers):
            x_before = x_current
            x_current = layer(x_current, t_emb, input_pos, freqs_cis, mask, None, None, None, None)
            tracker.track_mlx_module(f'transformer_layer_{i}', {'x_before': x_before, 't_emb': t_emb}, x_current)
            print(f"transformer_layer_{i}: {x_current.shape}, min={x_current.min():.6f}, max={x_current.max():.6f}, mean={x_current.mean():.6f}")
        
        # Transformer最终输出
        x_res = transformer.norm(x_current, t_emb)
        tracker.track_mlx_module('transformer_final', {'x_current': x_current, 't_emb': t_emb}, x_res)
        print(f"transformer_final: {x_res.shape}, min={x_res.min():.6f}, max={x_res.max():.6f}, mean={x_res.mean():.6f}")
        ...
        print("\n--- MLX WaveNet 跳过详细跟踪 ---")
        # 直接使用estimator的forward方法，跳过WaveNet内部细节
        wavenet_out = mx.zeros_like(x_out)  # 临时占位符
        tracker.track_mlx_module('wavenet_output', {'x_out': x_out}, wavenet_out)
        print(f"wavenet_output: {wavenet_out.shape}, min={wavenet_out.min():.6f}, max={wavenet_out.max():.6f}, mean={wavenet_out.mean():.6f}")
        ...
        x_final_input = x_final
        t_emb_input = t_emb
        tracker.track_mlx_module('final_layer_input', {'x_final': x_final_input, 't_emb': t_emb_input}, x_final_input)
        print(f"final_layer_input: {x_final_input.shape}, min={x_final_input.min():.6f}, max={x_final_input.max():.6f}, mean={x_final_input.mean():.6f}")
        ...
        # 12. conv2 - 调整输入格式
        final_out_transposed = final_out.transpose(0, 2, 1)  # (batch, seq_len, channels) -> (batch, channels, seq_len)
        output = mlx_estimator.conv2(final_out_transposed)
    ...
    modules_to_compare = [
        ...
        'cond_x_merge_linear', 'transformer_final', 'conv1', 'wavenet_output',
        'residual_connection', 'final_layer', 'conv2', 'final_output'
    ]
    
    # 添加Transformer层对比
    for i in range(13):  # 假设有13层
        modules_to_compare.append(f'transformer_layer_{i}')
    
    # 添加WaveNet层对比
    for i in range(8):  # 假设有8层
        modules_to_compare.append(f'wavenet_in_layer_{i}')
    ...
    # 保存详细结果
    results_file = "comprehensive_cfm_modules_comparison.pkl"
