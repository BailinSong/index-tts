#!/usr/bin/env python3
"""
CFM PyTorch vs MLX 子流程对比分析 (修复版)
...
def compare_pytorch_mlx_cfm_fixed():
    """对比 PyTorch 和 MLX CFM 实现 (修复版)"""
    print("🔍 PyTorch vs MLX CFM Comparison (Fixed)")
    print("="*60)
    ...
    # 检查输入数据问题
    print("\n🔍 Input Data Analysis:")
    analyze_input_data_issues(cached_inputs)
    
    # 收集 PyTorch CFM 子流程数据
    print("\n🔍 Collecting PyTorch CFM Submodule Data:")
    pytorch_data = collect_pytorch_cfm_data_fixed(cached_inputs)
    
    # 收集 MLX CFM 子流程数据
    print("\n🔍 Collecting MLX CFM Submodule Data:")
    mlx_data = collect_mlx_cfm_data_fixed(cached_inputs)
    
    # 对比分析
    print("\n📊 Submodule Comparison Analysis:")
    comparison_results = compare_submodule_data(pytorch_data, mlx_data)
    
    # 生成对比报告
    print("\n📄 Generating Comparison Report:")
    generate_comparison_report(cached_inputs, pytorch_data, mlx_data, comparison_results)
...
def analyze_input_data_issues(cached_inputs):
    """分析输入数据问题"""
    print("\n   📋 Input Data Issues:")
    ...
    print(f"     x (noise): range=[{x.min():.6f}, {x.max():.6f}]")
    print(f"     prompt_x: range=[{prompt_x.min():.6f}, {prompt_x.max():.6f}]")
    
    if x.min() == 0.0 and x.max() == 0.0:
        print(f"     ❌ Problem: x (noise) is all zeros!")
        print(f"     💡 This explains why output is all zeros")
    
    if prompt_x.min() != 0.0 or prompt_x.max() != 0.0:
        print(f"     ✅ prompt_x has non-zero values")
    
    # 检查 CFM 的处理逻辑
    print(f"\n     🔍 CFM Processing Logic:")
    print(f"     - CFM sets x[..., :prompt_len] = 0 (prompt region)")
    print(f"     - CFM sets prompt_x[..., :prompt_len] = prompt[..., :prompt_len]")
    print(f"     - If x is all zeros, output will be zeros after Euler steps")


def collect_pytorch_cfm_data_fixed(cached_inputs):
    """收集 PyTorch CFM 子流程数据 (修复版)"""
    try:
        # 导入 PyTorch CFM
        ...
        # 手动执行 CFM 逻辑来观察中间步骤
        print(f"\n     🔍 Manual CFM Execution:")
        
        # 应用 prompt (这是 CFM 的关键步骤)
...
def collect_mlx_cfm_data_fixed(cached_inputs):
    """收集 MLX CFM 子流程数据 (修复版)"""
    try:
        # 导入 MLX CFM
...
def compare_submodule_data(pytorch_data, mlx_data):
    """对比子模块数据"""
...
def generate_comparison_report(cached_inputs, pytorch_data, mlx_data, comparison_results):
    """生成对比报告"""
    report_lines = []
    report_lines.append("# PyTorch vs MLX CFM 子流程对比分析报告 (修复版)")
    report_lines.append("="*60)
    report_lines.append("")
    
    report_lines.append("## 🎯 分析目标")
    report_lines.append("")
    report_lines.append("使用 CFM 前级缓存数据，分别运行 PyTorch 和 MLX 版本的 CFM 实现，")
    ...
    if x.min() == 0.0 and x.max() == 0.0:
        ...
        report_lines.append("")
        report_lines.append("**影响**: 这解释了为什么输出全为零")
        report_lines.append("")
        report_lines.append("**CFM 处理逻辑**:")
        report_lines.append("1. CFM 设置 `x[..., :prompt_len] = 0` (prompt 区域)")
        report_lines.append("2. CFM 设置 `prompt_x[..., :prompt_len] = prompt[..., :prompt_len]`")
        report_lines.append("3. 如果 `x` 全为零，Euler 步骤后输出仍为零")
        report_lines.append("")
    else:
        report_lines.append("✅ **输入数据正常**: `x` 包含非零值")
        report_lines.append("")
    ...
    if pytorch_data.get('success'):
        ...
        if 'x_processed' in pytorch_data:
            x_processed = pytorch_data['x_processed']
            report_lines.append(f"- 处理后 x 范围: [{x_processed.min():.6f}, {x_processed.max():.6f}]")
    else:
    ...
    if mlx_data.get('success'):
        ...
        if 'x_processed' in mlx_data:
            x_processed = mlx_data['x_processed']
            report_lines.append(f"- 处理后 x 范围: [{float(x_processed.min()):.6f}, {float(x_processed.max()):.6f}]")
    else:
        report_lines.append("❌ **数据收集失败**")
    ...
    report_lines.append("### 3. 输入数据修复")
    report_lines.append("")
    report_lines.append("**问题**: 缓存输入数据中 `x` 全为零")
    report_lines.append("")
    report_lines.append("**解决方案**:")
    report_lines.append("1. 使用真实的噪声数据而不是零初始化")
    report_lines.append("2. 确保输入数据包含有意义的数值")
    report_lines.append("3. 验证 CFM 的 prompt 处理逻辑")
    report_lines.append("")
    ...
    # 保存报告
    report_path = Path("cfm_debug_outputs/pytorch_mlx_cfm_comparison_report_fixed.md")
...
def main():
    ...
    try:
        ...
        compare_pytorch_mlx_cfm_fixed()
        
        print("\n🎉 Comparison completed successfully!")
        print("\n📋 Generated files:")
        print("1. cfm_debug_outputs/pytorch_mlx_cfm_comparison_report_fixed.md - PyTorch vs MLX CFM 对比分析报告 (修复版)")
